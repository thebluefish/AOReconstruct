...
MainWindowBase::MainWindowBase( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxFrame( parent, id, title, pos, size, style )
{
	...
	_scanButton = new wxButton( _reconstituteTab, wxID_ANY, wxT("Reconstitute"), wxDefaultPosition, wxDefaultSize, 0 );
	_scanButton->Enable( false );
	
	bSizer18->Add( _scanButton, 0, wxALIGN_CENTER|wxALL, 5 );
	...
	m_notebook1->AddPage( _reconstituteTab, wxT("Reconstitute"), false );
	...
	m_notebook1->AddPage( _logTab, wxT("Log"), true );
	...
	// Connect Events
	_newPSTPicker->Connect( wxEVT_COMMAND_FILEPICKER_CHANGED, wxFileDirPickerEventHandler( MainWindowBase::_pickerPSTOnFileChanged ), NULL, this );
...
{
	// Disconnect Events
	_newPSTPicker->Disconnect( wxEVT_COMMAND_FILEPICKER_CHANGED, wxFileDirPickerEventHandler( MainWindowBase::_pickerPSTOnFileChanged ), NULL, this );
